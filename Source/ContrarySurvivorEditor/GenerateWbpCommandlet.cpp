...
namespace
{
	...
	// WBP_Consent: тексты согласия. ⛔ ИСТОРИЯ ОШИБКИ (24.08.2026): прежняя версия считала
	// правдой C++-структуру FConsentScreenStyle и ПЕРЕЗАПИСЫВАЛА непустые подписи ассета —
	// прогон 917749a затёр формулировки, которые Ринат правил руками в дизайнере (его
	// коммит 0489ac0). Ринат: «Я ЭТОТ ТЕКСТ ПРАВИЛ В WBP! КАКОГО ... В СБОРКУ ИДУТ НЕ МОИ
	// WBP!». Решение лида: ПРАВДА ФОРМУЛИРОВОК — АССЕТ ВЛАДЕЛЬЦА (ADR-077 п.0), прежнее
	// требование издателя 13.08 «тексты дословно из настроек» отменено.
	//
	// Теперь augment ЗАПОЛНЯЕТ ТОЛЬКО ПУСТЫЕ подписи (свежесобранный ассет/новый кубик) —
	// непустой текст принадлежит владельцу и не трогается никогда (правило владельческих
	// правок, см. комментарий у RebuildAssets). Рантайм зеркально: ApplyStyle ставит текст
	// только в пустой кубик дизайнер-дерева (ConsentScreenWidget.cpp).
	//
	// Идемпотентно: после первого заполнения кубики непусты — прогон ничего не меняет.
	void AugmentConsentTexts(UWidgetTree* Tree, bool& bChanged)
	{
		const TCHAR* Name = TEXT("WBP_Consent");
		const FConsentScreenStyle Texts; // источник ТОЛЬКО для пустых кубиков
		...
		for (const FConsentLabel& Label : Labels)
		{
			...
			{
				// Непустой текст — собственность владельца ассета: НЕ перезаписываем,
				// даже если он отличается от C++-дефолта.
				continue;
			}
			Block->SetText(Label.Text);
			bChanged = true;
			UE_LOG(LogGenerateWbp, Display,
				TEXT("AUGMENT %s: ПУСТАЯ подпись «%s» заполнена текстом из настроек экрана."),
				Name, Label.CubeName);
		}
	}

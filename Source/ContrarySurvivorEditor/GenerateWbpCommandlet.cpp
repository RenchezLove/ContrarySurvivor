...
namespace
{
	...
	bool BuildStartScreen(UWidgetTree* Tree)
	{
		...
		if (UCanvasPanelSlot* LogoSlot = Root->AddChildToCanvas(Logo))
		{
			...
			// «МАРЕВО» обрезана по надписи, 1024 на 360 (около 2.84 к 1): при ширине 420
			// высота 148. Квадрат 420x420 растягивал надпись по высоте почти втрое.
			LogoSlot->SetSize(FVector2D(420.0f, 148.0f));
		}

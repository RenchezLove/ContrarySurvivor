 ...
 *
 * ВОЗРОЖДЕНИЕ И СТУПЕНИ (ТЗ издателя 22.08.2026, бэклог №31/№35): база помнит зачистку по
 * РЕАЛЬНЫМ часам в сейве и возрождается на следующей ступени (цикл 1→…→5→3), враги ступени
 * получают прибавку здоровья ОТ БАЗЫ, при входе показывается надпись, занятая база слышна
 * заранее. РЕШЕНИЕ лида 22.08: ДОБЫЧА ЗА ЗАЧИСТКУ — МЕШОК-ПИКАП В ЦЕНТРЕ БАЗЫ (штатный
 * APickup, то же окно обыска), а НЕ лут с тел: трупы дают свой обычный лут, награда ступени
 * лежит отдельно и читает единый формат предметов (строки DT_Items). Правила ступеней —
 * чистая логика EnemyBaseTierLogic.h (гоняется headless-тестом EnemyBaseRespawnTests).
 */
...
protected:
	...
	// логово/сарай. «Отдельное хранилище» со StashLevel > 0: при прямом обыске — своё окно,
	// но в группу якоря-трупа/мешка ВХОДИТ членом (решение лида 24.08, ADR-077 п.14 — база
	// обыскивается сразу по смерти последнего врага, награда в одном окне с трупами). После
	// обыска база никуда не девается. Наполняется в FillBaseStash; настройки скрыты — правда
	// лута в «Награда по ступеням». Без VisibleAnywhere намеренно (урок п.9 машины: секции
...
public:
	...
	// «Новая игра» посреди ЖИВОГО мира (баг Рината 23.08: после новой игры волки в логове
	// не заспавнились). Выбор «Новая игра» на стартовом экране/в настройках происходит в уже
	// загруженном мире: BeginPlay базы к этому моменту давно прочитал СТАРЫЙ сейв, и стирание
	// слота (APlayerCharacter::ResetToNewGame) память в акторе само не обнуляет — база стояла
	// бы «зачищена, пауза тикает» и врагов не давала. Возвращает базу в исходное состояние:
	// начальная ступень, зачисток не было, взведена; живые враги старой сессии, старый
	// квест-предмет и лут хранилища убираются. Смерть/«Продолжить» память баз ЗАКОННО
	// сохраняют (ТЗ издателя 22.08 §1) — сброс делает ТОЛЬКО явная «Новая игра».
...
private:
	...
	// Пикап квест-предмета, заспавненный ЭТОЙ базой (слабая ссылка: подобранный/истёкший
	// отмирает сам). Нужен «Новой игре»: не подобранный старый ноутбук убирается, иначе
	// возрождённая база при первой активации положила бы рядом второй.
	TWeakObjectPtr<APickup> SpawnedQuestPickup;
	...
	// «эти выглядят…» и полупрозрачная цифра — ТОЛЬКО когда база занята (bBaseOccupied).
	// Пустая зачищенная база честно остаётся «Лагерь бандитов» без пугалок.
	void ShowEntryAnnounce(bool bBaseOccupied);

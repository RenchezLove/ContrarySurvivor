	...
	UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	// ⛔ ФИКС КРИТИЧЕСКОГО бага сейвов (ADR-076 п.1): «Продолжить» телепортирует игрока в
	// зону костра ДО того, как память полностью восстановлена, и этот overlap срабатывает
	// прямо внутри телепорта — автосейв здесь перезаписывал слот пустым состоянием (второй
	// перезапуск стирал прогресс, флаги интро выживали через CopyRetentionData). Пока игрок
	// в процессе восстановления — автосейв пропускаем; антиспам-таймер не трогаем, чтобы
	// первый НАСТОЯЩИЙ вход в зону сохранил как обычно.
	if (Player->IsRestoringSaveState())
	{
		UE_LOG(LogQA, Display,
			TEXT("QA: автосейв костра ПРОПУЩЕН — игрок восстанавливается из «Продолжить» (ADR-076 п.1)"));
		return;
	}

 ...
 *
 * ПЕТЛИ СТВОРОК: угол применяется поворотом вокруг точки-петли (поле «петля» у каждой
 * створки, в локальных координатах детали). Ноль-вектор = вращение вокруг пивота меша.
 * Если пивоты деталей окажутся в нуле машины (экспорт «по месту»), оператор задаёт петли
 * руками — код не зависит от того, где моделлер оставил пивот.
 * ⚠ Код ПЕРЕЗАПИСЫВАЕТ RelativeLocation/Rotation створок и стёкол из полей угла/петли —
 * двигать их мышкой в BP бесполезно, положение задаётся ТОЛЬКО полями (иначе OnConstruction
 * копил бы повороты). Колёса и блок код не двигает — их расставляет оператор в BP.
 *
...
protected:
	...
	// меша. Если пивот детали не на петле — оператор вписывает точку петли здесь. ===
...
private:
	...
	// Поворот створки на AngleDeg вокруг точки-петли HingeLocal (локальные координаты детали).
	// bYaw: двери — вокруг вертикали, капот/багажник — вокруг поперечной оси (тангаж).
	// ПЕРЕЗАПИСЫВАЕТ RelativeLocation/Rotation детали (см. шапку класса).
	static void ApplyHinge(UStaticMeshComponent* Part, const FVector& HingeLocal, float AngleDeg, bool bYaw);

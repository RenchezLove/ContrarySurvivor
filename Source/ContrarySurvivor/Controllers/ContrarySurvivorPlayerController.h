...
public:
	...
	// ⛔ Какой класс окна брать: назначенный владельцем ассет либо C++-запаска. Ассет пропал
	// или слот не заполнен — окно ОБЯЗАНО собраться кодом, а не остаться пустотой. Правило
	// вынесено отдельной чистой функцией именно затем, чтобы его гонял автотест: сломаться
	// оно может молча, а заметно это станет только на устройстве.
	static UClass* ResolveSupportWidgetClass(UClass* AssignedClass);
...
protected:
	...
	// они клампят и бродкастят делегаты, HUD обновляется сам).
	void OnQAFullStats();
	...
	// Сколько денег выдаёт клавиша денег (K) за нажатие (DRAFT, тюнингуется).
...
private:
	...
	// QA: дебаунс тумблера god-mode (T). Отчёт тестера «GODMODE сам выключился сразу после
	...
	// Завести и остановить трек меню (внутренние; снаружи зовут ApplyMenuMusicGate).
	void StartPauseMusic();
	void StopPauseMusic();

	// Единственное место решения «играть ли сейчас музыку меню»: трек звучит, пока на экране
	// пауза, главное меню или настройки поверх них (просьба Рината 08-09 — музыка и в меню).
	// Идемпотентен: зовётся по каждому событию открытия и закрытия, лишнего не делает.
	void ApplyMenuMusicGate();
	...
	// Последний срез «что нашли в мире и почему пропустили» — строка в журнал пишется только
	// при его смене (разбор ночной сессии 08-09: заглушать было нечего, а звук шёл).
	FString WorldAudioLastSnapshot;
	...
	// ⚠ УСТАРЕЛО (ADR-076 п.2, решение Рината): радиус группового обыска теперь НАСТРОЙКА
	// НА BP ИГРОКА (APlayerCharacter::CorpseGroupSearchRadius) — контроллер читает его с
	// пешки (GetCorpseGroupSearchRadiusEffective). Это поле — запасной путь, когда пешки
	// нет; не удалено, чтобы не потерять возможную правку в BP контроллера (сверка дампом
	// за лидом), удаление — отдельной волной.
	UPROPERTY(EditAnywhere, Category = "Interact", meta = (ClampMin = "0.0",
		DisplayName = "УСТАРЕЛО: радиус группы (источник — BP игрока)"))
	float CorpseGroupSearchRadius = 600.0f;

	// Действующий радиус группового обыска: поле ИГРОКА (ADR-076 п.2), фолбэк — поле выше.
	float GetCorpseGroupSearchRadiusEffective() const;
	...
	// ADR-074: одноразовая подсказка «прогресс сохраняется только у костра» при ПЕРВОМ ВЫХОДЕ
	// из деревни. Зовётся из Tick рядом с проверкой подсказок подбора: следит за границей
	// AVillageZone и стреляет ровно на переходе «был внутри → вышел» (см. bWasInsideVillage).
	// Во время интро не проверяет: игра начинается за деревней, подсказка на старте не нужна.
	void UpdateLeaveVillageHint();

	// Игрок в прошлый проверенный кадр стоял ВНУТРИ деревни. Без этого признака подсказка
	// сработала бы просто по факту «стоит снаружи» — то есть сразу на старте новой игры.
	bool bWasInsideVillage = false;

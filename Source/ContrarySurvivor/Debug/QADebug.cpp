...
void FQADebug::QA(const UObject* /*WorldCtx*/, const FString& Msg, bool bScreen)
{
	...
	//    В публикационной сборке оверлея нет вовсе (Б5), поэтому и буфер не копим — иначе
	//    строки собирались бы в память, которую никто никогда не покажет.
	if (!bScreen || !CONTRARY_WITH_QA_CHEATS)
	{
		return;
	}

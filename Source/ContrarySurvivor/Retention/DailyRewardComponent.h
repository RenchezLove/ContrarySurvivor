...
private:
	...
	// --- Build 1.2: «Забрать вдвое больше» за просмотр ролика (ТЗ издателя №3) ---

	// Клик по золотой кнопке: аналитика + показ «ролика» через слой IAdService.
	void HandleDoubleRequested();

	// Досмотрен: доначислить ещё одну дневную сумму (итог = двойная), зеркало в сейв,
	// итог в окне. Начисление ДО закрытия баннера (ТЗ №3 п.5: награда не должна пропасть).
	void HandleDoubleAdSuccess();

	// Закрыт досрочно: ничего не начислять, окно в исходном состоянии, повтор разрешён.
	void HandleDoubleAdFail();
	...
	// Обычная награда, выданная при показе окна (база удвоения этого дня).
	float GrantedReward = 0.0f;

	// День серии выданной награды (параметр аналитики удвоения).
	int32 GrantedStreak = 0;

	// «Ролик» уже идёт / удвоение этого дня уже получено (двойной клик и повторное
	// удвоение исключены).
	bool bDoubleAdInProgress = false;
	bool bDoubleClaimed = false;
};

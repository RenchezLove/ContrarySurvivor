...
public:
	...
	// Иконка по СЛУЖЕБНОМУ КЛЮЧУ ItemName (Build 1.2.2, тайлы): один класс обслуживает и
	// шкуру, и ноутбук — класс-дефолт их различить не может, а ключ стабилен по контракту
	// (ADR-050: значения не менять, по ним сходится зачёт квеста). Покрывает все пути
	// создания: дроп волка, спавн ноутбука, размещённый пикап. Явный ItemIcon главнее;
	// незнакомый ключ — иконки нет (тайл покажет только подпись).

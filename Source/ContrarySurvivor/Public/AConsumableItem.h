...
public:
	...
	// ADR-088: прошитые в коде пути картинок по типу (GetDefaultIcon) убраны — картинка
	// расходника приходит только из его строки таблицы предметов (ItemIcon, базовый геттер).

	// Название для игрока (фикс 08-07 «тушёнка называется "Предмет"»): заполненные поля
	// главнее (Super), но если спавнер не заполнил НИ названия, НИ ключа (пикап лагеря
	// с голым классом расходника) — показываем штатное название своего типа
	// («Консервы»/«Вода»/«Аптечка») вместо заглушки «Предмет».
	virtual FText GetItemDisplayText() const override;

protected:
	// ADR-088: ключ, заданный на карте по-старому, может не совпасть с таблицей («Тушёнка»).
	// Тогда строка ищется по штатному ключу ТИПА («Консервы» -> canned_food). Совсем без
	// ключа строку НЕ угадываем: такой предмет ещё не знает, кем он будет.
	virtual FName ResolveItemRowName() const override;
};

...
public:
	...
	// Имя строки DT_Items, из которой собран ЭТОТ экземпляр (заполняет ContraryItems::
	// ApplyRowToItem). Класс предмет НЕ определяет: одна BP_ArmorBase обслуживает все девять
	// броней, и слот/защита/меш экипировки приходят из строки. Ключ уезжает в сейв и при
	// «Продолжить» строка накладывается заново — иначе восстановленный предмет получает
	...
	// Виртуальный (фикс 08-07 «тушёнка называется "Предмет"»): класс, умеющий вывести имя
	// из своих данных, подставляет его ВМЕСТО заглушки (AConsumableItem — по типу расходника).
	UFUNCTION(BlueprintPure, Category = "Item")

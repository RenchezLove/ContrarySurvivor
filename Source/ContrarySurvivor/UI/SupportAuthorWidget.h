...
struct FSupportAuthorStyle
{
	...
	// лида. Показывается ТОЛЬКО вместе со строкой благодарности и только пока следующий ролик
	// не готов; при обычном открытии окна без готового ролика её нет — окно с одной кнопкой
	// не шумит (требование издателя).
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Поддержать автора|Тексты",
		meta = (MultiLine = "true"))
	FText NextAdHintText = NSLOCTEXT("SupportAuthorWidget", "NextAdHint",
		"Новый ролик будет готов через 30–60 секунд — загляните в это окно ещё раз.");
...
public:
	...
	// Показывать ли подсказку «новый ролик будет готов через 30–60 секунд». Ровно два условия:

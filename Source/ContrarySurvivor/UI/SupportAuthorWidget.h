...
struct FSupportAuthorStyle
{
	...
	// исчезает, пока следующий не загрузится, и игрок её теряет). Текст дословно из поправки
	// Рината. ⛔ «Несколько секунд», а не «30–60»: журнал телефона показал, что следующий
	// ролик подгружается меньше чем за секунду (phone-1313-b2.log:1233–1258) — числа были бы
	// неправдой. Показывается ТОЛЬКО вместе со строкой благодарности и только пока следующий
	// ролик не готов; при обычном открытии окна без готового ролика её нет — окно с одной
	// кнопкой не шумит (требование издателя).
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Поддержать автора|Тексты",
		meta = (MultiLine = "true"))
	FText NextAdHintText = NSLOCTEXT("SupportAuthorWidget", "NextAdHint",
		"Новый ролик будет готов через несколько секунд — загляните в это окно ещё раз.");
...
public:
	...
	// Показывать ли подсказку «новый ролик будет готов через несколько секунд». Ровно два условия:

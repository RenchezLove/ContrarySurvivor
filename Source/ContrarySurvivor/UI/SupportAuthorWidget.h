 ...
 *
 * ⛔ С 24.08.2026 ТЕКСТЫ ИЗ ЭТИХ ПОЛЕЙ — ТОЛЬКО ЗАПАСКА: правда формулировок живёт в ассете
 * владельца (решение лида, ADR-077 п.0 «всё в WBP»; прежнее правило «тексты всегда из
 * настроек» отменено — оно молча затирало ручные правки Рината). В дереве из дизайнера код
 * ставит текст ТОЛЬКО в пустой кубик; непустой текст не трогается никогда.
 *
...
struct FSupportAuthorStyle
{
	...
	// --- Тексты-запаска. Идут в игру, только если окна из дизайнера нет ЛИБО владелец оставил
	// кубик пустым (см. предупреждение в шапке структуры). ⛔ Ограничение издателя действует на
	// ЛЮБОЙ источник текста, включая ассет: ни сроков, ни слова «скоро», ни обещаний
	// бесплатности, ни назначения денег (ADR-072) — за формулировку в ассете отвечает его
	// владелец, код за него не подставляет и не правит. ---
 ...
 *    дизайнера, кубики приходят по именам, код не перекрашивает и НЕ ПЕРЕПИСЫВАЕТ ТЕКСТЫ;
 *  - ассета нет / слот пуст → кодовое дерево-запаска, оформление из FSupportAuthorStyle.
 *
 * ⛔ КОНТРАКТ ПО ТЕКСТУ (решение лида 24.08.2026, после того как код второй раз показал в игре
 * не то, что владелец написал в ассете). Правда формулировок — АССЕТ ВЛАДЕЛЬЦА. В дереве из
 * дизайнера код заполняет подпись только тогда, когда кубик ПУСТ (свежесобранный ассет или
 * только что добавленный кубик), и не перезаписывает непустую подпись НИКОГДА — ни при
 * применении настроек, ни при показе благодарности после ролика. Ограничение издателя из
 * списка выше при этом никуда не делось, просто соблюдает его теперь автор текста в ассете:
 * код чужую формулировку не чинит и не подменяет.
 *
 * ⚠ Тексты этого окна по смыслу СТАТИЧНЫЕ: ни одна подпись не должна меняться по ходу показа
 * окна — благодарность и подсказка про следующий ролик появляются и исчезают ТОЛЬКО
 * видимостью (SetVisibility), а слова в них всё время одни и те же. Видимость — механика,
 * она целиком за кодом и работает независимо от того, чей текст в кубике. Если когда-нибудь
 * понадобится подпись, которая по ходу МЕНЯЕТ слова (счётчик, имя, число), её нельзя делать
 * перезаписью кубика владельца: правильный путь — либо отдельный кубик под изменяемую часть,
 * либо шаблон с подстановкой (FText::Format), взятый ИЗ текста ассета.
 *
...
public:
	...
	// ⛔ Метод только ПОКАЗЫВАЕТ строку. Слова в ней — из ассета владельца и здесь не
	// переписываются (см. контракт по тексту в шапке класса).
	void ShowThanks();
...
private:
	...
	// ⛔ ЕДИНСТВЕННАЯ дверь, через которую код пишет текст в подпись окна (контракт по тексту
	// в шапке класса). Пишет ровно в двух случаях: дерево построили мы сами (дизайнера нет,
	// и запаске нужны слова) ЛИБО кубик владельца пуст. Непустую подпись из ассета не трогает.
	//
	// Почему опора на bCodeTreeBuilt, а не на bDesignerTree: признак дизайнера ставится в
	// NativeOnInitialized, а движок 5.5 зовёт её только при живом игровом контексте
	// (UserWidget.cpp:159-163). Не отработала — bDesignerTree ещё false, и текст владельца
	// был бы перезаписан. Признак «мы это построили» безопасен в обе стороны.
	void SetTextIfCodeOwns(UTextBlock* Block, const FText& Text) const;

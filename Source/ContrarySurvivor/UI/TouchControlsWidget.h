...
public:
	...
	// их слоты: только видимость (галка ниже), поворот стрелки на север и Render Translation
	// вслед за стиком (тот же приём, что ход «шляпки»), когда замок фикс-отступа двигает
	// стик при смене разрешения. ---
...
protected:
	...
	// --- Кубики компаса — ИЗ АССЕТА (-augment; П.0 ADR-077: код их не создаёт и слоты не
	// двигает — Ринат раскладывает и стилизует в дизайнере). Нет в ассете — предупреждение
	// в лог, компаса не будет до прогона генератора. ---
...
private:
	...
	// Компас (П.0 ADR-077 — слоты дизайнера НЕ трогаются): видимость по bShowCompass и
	// поворот стрелки на север (угол — проекцией NorthWorldDirection через камеру).
	// Зовётся каждый кадр вне модалок; поворот применяется только при заметной смене угла.
	void UpdateCompass();

	// Гейт поворота стрелки (1e6 = угол ещё не считался).
	float LastCompassAngleDeg = 1.0e6f;

	// Компас скрыт галкой bShowCompass (гейт, чтобы не дёргать видимость каждый кадр).
	bool bCompassHiddenBySetting = false;

	// Дизайнерская позиция слота подложки стика — снимок ДО первой правки замком фикс-отступа
	// (ApplyStickCornerLock): по ней считается Render Translation компаса вслед за стиком.
	FVector2D DesignerStickPos = FVector2D::ZeroVector;
	bool bDesignerStickPosCached = false;

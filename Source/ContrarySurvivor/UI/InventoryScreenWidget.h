...
public:
	...
	// UCorpseLootWidget при Search, UShopScreenWidget при Trade; при Stash пока не
	// используется — задел). НЕ Normal — кнопка закрытия рюкзака прячется (её роль в паре
	// берёт окно-напарник слева, ТЗ п.10); Normal — кнопка возвращается к видимости, снятой
	// с кубика при инициализации. Партнёра храним слабой ссылкой — окно-напарник может
	// закрыться/умереть раньше рюкзака.
	void SetPanelMode(EItemPanelMode InMode, UObject* InPartner);

	// Гасит/включает интерактивность ВСЕГО окна целиком (ADR-082, этап 3): пока у
	// окна-напарника открыта модальная панель количества (сделка магазина), клики по слотам
	// брони/оружия и по плиткам рюкзака должны молчать — иначе игрок кликнет по рюкзаку
	// поверх открытой панели. SetIsEnabled каскадом выключает весь Slate-поддерево (тот же
	// приём, что BuyList/SellList->SetIsEnabled(false) в UShopScreenWidget).
	void SetInteractionEnabled(bool bEnabled) { SetIsEnabled(bEnabled); }

	// Публичная пересборка (ADR-082): окно-напарник просит пересчёт после сделки/переноса
	// (деньги и состав рюкзака сменились). Сама логика — RefreshAll ниже (protected, зовётся
	// и изнутри).
	void RefreshInventoryDisplay() { RefreshAll(); }
...
protected:
	...
	// клик мимо кнопок»). Правило ADR-084: в паре окон затемнение остаётся ровно у нижнего.
	// Search/Stash — инвентарь верхний, свой кубик прячем (иначе ловит клики по обыску).
	// Trade (ADR-085) — магазин слоем выше ради модальной панели количества, инвентарь
	// нижний, затемнение остаётся у него (магазин гасит своё).
	UPROPERTY(meta = (BindWidgetOptional))

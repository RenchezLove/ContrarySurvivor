...
struct FStartScreenStyle
{
	...
	// ⚠ Пропорция обязана совпадать с пропорцией картинки, иначе надпись растянет. Живая
	// текстура логотипа «МАРЕВО» обрезана по надписи и имеет 1024 на 360 (примерно 2.84 к 1).
	// Размер выбран game-lead по макету Рината: название занимает около трети ширины экрана,
	// поэтому 700 при эталонной ширине 1920, и высота 246 из той же пропорции. Прежний
	// квадрат 420x420 растягивал надпись втрое, а 420x148 давал верную форму, но мелко.
	// Меняешь ширину — пересчитай высоту как ширина делить на 2.84.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Start Screen|Раскладка",
		meta = (DisplayName = "Размер логотипа"))
	FVector2D LogoSize = FVector2D(700.0f, 246.0f);
...
private:
	...
	// Геометрия кодового дерева по полям стиля: отступ логотипа, отступ колонки кнопок от
	// правого края, зазор между кнопками, полосы затемнения внизу. Только для кодового пути —
	// в живом WBP раскладку двигает владелец.
	void ApplyCodeTreeLayout(const FStartScreenStyle& Style);

	// Размер ячейки логотипа из LogoSize — в ОБОИХ путях, включая дерево из дизайнера.
	//
	// Это осознанное исключение из правила «в дизайнерском дереве геометрию не трогаем».
	// Обоснование: размер логотипа обязан держать пропорцию текстуры (у нашей надписи она
	// около 2.84 к 1), иначе картинку растянет — то есть это не вкусовая раскладка, а
	// техническая связка «картинка и её рамка». Без этого размер логотипа менялся бы только
	// пересборкой окна служебной программой, и Ринат не мог бы подкрутить его сам.
	// ⛔ Трогаем ТОЛЬКО ячейку логотипа. Ничего другого в дизайнерском дереве код не двигает.
	void ApplyLogoCellSize(const FStartScreenStyle& Style);

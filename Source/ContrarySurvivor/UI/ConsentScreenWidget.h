 ...
 * данных, которые названы и в ОПУБЛИКОВАННОЙ политике, — сегодня это рекламная сеть
 * Яндекса и GameAnalytics.
 *
 * ⛔ AppMetrica ИЗ ТЕКСТА УБРАНА (требование издателя 13.08.2026): опубликованная политика
 * прямо говорит, что она не используется, а экран согласия утверждал обратное — документ и
 * игра обязаны совпадать. Прежнее требование называть её (задание Б6) отменено. Сама
 * библиотека при этом физически едет в пакете как зависимость рекламного набора Яндекса,
 * но наш код её не поднимает и ключа у неё нет — сборщиком данных для игрока она не является.
 *
...
struct FConsentScreenStyle
{
	...
	FText BodyText2 = NSLOCTEXT("ConsentScreenWidget", "BodyText2",
		"Этим занимаются рекламная сеть Яндекса и GameAnalytics. Имя, телефон, почта, контакты и точное местоположение НЕ собираются никогда.");
 ...
 *    и с 24.08 ТЕКСТЫ — ТОЖЕ ВЛАДЕЛЬЦА: Ринат правит формулировки прямо в ассете
 *    (ADR-077 п.0 «всё в WBP»), код ставит текст только в ПУСТОЙ кубик. Прежнее правило
 *    «тексты всегда из настроек» отменено решением лида 24.08 — оно молча затёрло
 *    правку Рината (его коммит 0489ac0, затем прогон augment). За сходство формулировок
 *    ассета с опубликованной политикой (soglasie-i-politika.md) теперь отвечает владелец;

...
private:
	...
	// Дерево построили МЫ (запаска без WBP). ⛔ Только в нём код вправе писать поверх непустой
	// подписи: в окне владельца его текст не трогаем (UI/OwnerTextGuard.h).
	bool bCodeTreeBuilt = false;

	// Заголовок, с которым окно пришло из дизайнера. Обыск контейнера с собственным именем
	// временно ставит в кубик ЕГО имя (данные), и вернуть потом надо авторский заголовок.

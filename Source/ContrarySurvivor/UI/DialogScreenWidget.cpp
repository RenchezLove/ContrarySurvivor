...
void UDialogScreenWidget::BindCloseCross()
{
	// П.0 отчёта 23.08 (ADR-077): кодовое СОЗДАНИЕ крестика удалено — кубики добавляет
	// -augment генератора, Ринат двигает их в дизайнере. Код: обработчик, знак, видимость.
	if (!DialogCloseCrossButton)
	{
		UE_LOG(LogQA, Warning,
			TEXT("DialogScreenWidget: кубика DialogCloseCrossButton нет в WBP_Dialog — крестика закрытия не будет; прогоните генератор в режиме дополнения"));
		return;
	}
...
// Кодовый СТЕК кнопок (FixButtonsGeometry) УДАЛЁН — П.0 отчёта Рината 23.08 (ADR-077):
// именно он двигал кнопки в игре мимо дизайнера («в WBP всё по-старому, а в игре
// поехали» — кадр). Раскладка кнопок теперь ЦЕЛИКОМ дизайнерская; перенос длинных
// реплик — свойством переноса у кубиков подписи в ассете (ставит -augment генератора).

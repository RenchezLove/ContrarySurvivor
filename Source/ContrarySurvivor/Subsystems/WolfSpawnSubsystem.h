...
protected:
	...
	// южнее, симметрично). BugReport 12: базы отодвинуты ПОДАЛЬШЕ от деревни → дефолт
	// (0,+3500,Z), ≥2500 от края деревни. Точную высоту КАЖДОГО волка берём трассой до пола,
	// поэтому дефолтный Z тут не критичен. EditAnywhere — чтобы unreal-operator подвинул
	// Логово под реальную карту без перекомпиляции (координаты согласованы с оператором).
	UPROPERTY(EditAnywhere, Category = "WolfSpawn")
	FVector WolfDenLocation = FVector(0.0f, 3500.0f, 0.0f);

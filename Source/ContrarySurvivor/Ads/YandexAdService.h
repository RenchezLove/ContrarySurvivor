...
public:
	...
	// Четвёртая точка — окно «Поддержать автора». Имя поля согласовано с рекламным кабинетом
	// и менять его нельзя. Пока пусто: боевой идентификатор впишется в конфиг, когда придёт.
	UPROPERTY(Config)
	FString AdUnitIdSupportAuthor;

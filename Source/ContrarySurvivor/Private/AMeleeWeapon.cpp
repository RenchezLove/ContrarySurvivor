...
AMeleeWeapon::AMeleeWeapon()
{
	...
	// Урон удара. 08-09, живой осмотр Рината: «сделай урон от ножа чуть выше» — 35 -> 40.
	// Промежуточных значений тут нет: 36-39 не меняют НИ ОДНОГО исхода боя (волк 40 здоровья
	// всё так же переживает удар, бандит 80 — всё так же держит три), а 40 срезает по одному
	// удару и волку (было два, стало один), и бандиту (было три, стало два). Поле правится
	// в настройках оружия, откатить одним числом.
	Damage      = 40.0f;

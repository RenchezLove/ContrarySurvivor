...
protected:
	...
	// офсеты персонажа WeaponGripLocation/Rotation при сокете не участвуют, но ПОВЕРХ
	// сокета всегда применяется поправка КЛАССА ОРУЖИЯ (AMasterWeapon::GripOffset*,
	// Build 1.2.2 — у ножа своя поза, у пистолета нулевая). Сокета нет — прежний путь:
	// кость WeaponAttachBoneName + офсеты персонажа, поправка оружия поверх них.

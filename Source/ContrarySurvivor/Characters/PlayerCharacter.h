...
protected:
    ...
    // Радиус (см), в котором применённая из сейва точка считается «у костра» (решение
    ...
    // берём тройной запас. Проверяются оба пути применения сейва: смертельное возрождение
    // и «Продолжить» (08-07: испорченный слот старой сборки ронял игрока в поле и на старте).
...
public:
    ...
    // Точка из слота дальше RespawnNearCampfireRadius от костра (испорченный сейв старой
    // сборки) лечится перестановкой к костру — как при смертельном возрождении.
    ...
    // --- Работа с сохранением БЕЗ ЖИВОГО ПЕРСОНАЖА (переезд запуска на загрузочный уровень,
    // ADR-067 п.5). На загрузочном уровне пешки нет вовсе, а главное меню обязано знать, есть
    // ли что продолжать, и уметь стереть сохранение по кнопке «Новая игра». Логика та же самая:
    // проверка ниже — источник истины, а HasSaveGame() у живого персонажа зовёт именно её,
    // чтобы два места не разошлись. ---

    // Есть ли в слоте сохранение С РЕАЛЬНЫМ ПРОГРЕССОМ: файл слота существует И у объекта в
    // слоте взведён признак реальных данных (bHasData).
    static bool HasSaveGameInSlot(const FString& SlotName, int32 UserIndex);

    // То же для слота по умолчанию — им и играют (имя слота берётся из умолчаний класса).
    static bool HasDefaultSaveGame();

    // Стереть слот сохранения. Живых статов эта функция не касается намеренно: на загрузочном
    // уровне персонажа ещё нет, а в новом мире он рождается заново и в BeginPlay сам берёт
    // стартовые значения новой игры — ровно потому, что сохранения больше нет.
    static void DeleteSaveInSlot(const FString& SlotName, int32 UserIndex);
    static void DeleteDefaultSaveGame();

    // Имя и номер слота по умолчанию — для мест, где живой пешки нет. Значения читаются из
    // умолчаний класса, а не пишутся строкой второй раз.
    static FString GetDefaultSaveSlotName();
    static int32 GetDefaultSaveUserIndex();
    ...
    // Идёт восстановление «Продолжить» (LoadGameForContinue) прямо сейчас. ⛔ ФИКС
    // КРИТИЧЕСКОГО бага сейвов (ADR-076 п.1): восстановление ТЕЛЕПОРТИРУЕТ игрока в зону
    // костра (сейв-точка всегда у костра), телепорт синхронно жмёт триггер костра, и его
    // автосейв ПЕРЕЗАПИСЫВАЛ слот недовосстановленным состоянием — пустой рюкзак/журнал,
    // а флаги интро выживали через CopyRetentionData. Отсюда симптом Рината: первый
    // перезапуск играет по ещё живому слоту (и сам его потрошит), второй — стартует «с
    ...
    // Зовёт контроллер после каждой правки настройки — звук меняется сразу, без перезапуска.
...
protected:
    ...
    // Применённая из сейва точка обязана быть у костра: позицию в слот пишут только автосейв
    // костра и пере-сейв смерти (уже после этой перестановки), поэтому точка дальше
    // RespawnNearCampfireRadius от ближайшего костра — всегда испорченный слот старой сборки
    // либо фолбэк «сейва нет» на стартовое поле. Переставляет игрока к костру (в стороне от
    // огня, лицом к нему, Z по полу). Костёр ищется по классу ACampfire (не по имени
    // экземпляра). Мир без костра (служебный/тестовый) — точка не меняется. Зовётся из обоих
    // путей применения сейва: Respawn() (решение лида 08-06, вариант 1+3) и
    // LoadGameForContinue() (08-07: «Продолжить» с испорченным слотом ронял игрока в поле).
    // ContextTag — префикс строк лога («DEATH RESPAWN» / «CONTINUE»).
    void RelocateToCampfireIfSavedPointFar(const TCHAR* ContextTag);
...
private:
    ...
    // Поднят на время LoadGameForContinue (см. IsRestoringSaveState — фикс ADR-076 п.1:
    // автосейв костра от телепорта восстановления перезаписывал слот пустым состоянием).
    bool bRestoringSaveState = false;
...
public:
    // Приглушить/вернуть лесной фон на время главного меню (жалоба Рината 08-09: «в меню
    // играет Ambient локации, поют птицы»).
    //
    // ⛔ ПОЧЕМУ НАПРЯМУЮ, А НЕ ОБЩИМ ФИЛЬТРОМ. Фон заводится через SpawnSound2D, а этот путь
    // помечает звук ЗВУКОМ ИНТЕРФЕЙСА (GameplayStatics.cpp:1669). Из-за этого его не глушит
    // ни пауза мира, ни наш общий отбор звуков мира — он интерфейсные пропускает намеренно,
    // иначе онемели бы кнопки и музыка. Живая сессия это и показала: «найдено 1, заглушено 0,
    // пропущено: интерфейсных 1». Поэтому лесной фон глушим ПО ССЫЛКЕ, не полагаясь на признак.
    // Возвращает true, если состояние поменялось (для журнала владельца).
    bool SetAmbienceSilenced(bool bSilenced);

    // Молчит ли сейчас лесной фон (для автотестов и владельца).
    bool IsAmbienceSilenced() const { return bAmbienceSilenced; }

    // Множитель громкости лесного фона на время боя (задача №5, ТЗ 30.08: «звук уровня на
    // время боя приглушается на 30 %, после боя возвращается»). 1 — обычная громкость.
    // Зовёт контроллер игрока из опроса боевой музыки, плавность обеспечивает он же.
    void SetAmbienceCombatDuck(float InMultiplier);

private:
    // Фон приглушён нами на время меню (чтобы не трогать чужую паузу и не гасить дважды).
    bool bAmbienceSilenced = false;

    // Текущий боевой множитель громкости фона [0..1] (см. SetAmbienceCombatDuck). Участвует
    // и в ApplyAudioSettings — смена настроек громкости приглушение не перетирает.
    float AmbienceCombatDuck = 1.0f;

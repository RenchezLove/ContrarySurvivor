...
protected:
	...
	// ОСОБЫЕ цены выкупа по служебному ключу предмета (ItemName). Нужны вещам, которых в
	// прайс-листе нет и которые нельзя мерить категорией: шкура волка — квестовый предмет,
	// но живёт она в добыче охотника, и цена ей нужна своя. Ключ — служебный, НЕ переводится
	// (контракт ADR-050), берётся дословно из QuestLootItemName волка. Дыры «купил дешевле —
	// продал дороже» тут быть не может: такие вещи у торговца не купить.
	// Заполняется в конструкторе, тюнингуется в редакторе на размещённом торговце.
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Shop|Sell", meta = (DisplayPriority = "2"))
	TMap<FString, float> SpecialSellValues;

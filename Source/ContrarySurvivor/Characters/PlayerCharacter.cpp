...
bool APlayerCharacter::SaveGame()
{
    ...
    // ⛔ Вторая линия фикса ADR-076 п.1: пока идёт восстановление «Продолжить», писать слот
    // НЕЛЬЗЯ — в память ещё не всё вернулось, запись перезаписала бы сохранение
    // недовосстановленным состоянием (первую линию держит сам костёр, Campfire.cpp).
    if (bRestoringSaveState)
    {
        UE_LOG(LogQA, Warning,
            TEXT("QA: SaveGame ОТКЛОНЁН — идёт восстановление «Продолжить», запись затёрла бы слот (ADR-076 п.1)"));
        return false;
    }
...
bool APlayerCharacter::LoadGameForContinue()
{
    ...
    // ⛔ ФИКС КРИТИЧЕСКОГО бага сейвов (ADR-076 п.1, живой тест Рината «второй перезапуск
    // стирает сохранение»). КОРЕНЬ: ниже ApplySaveData/Relocate ТЕЛЕПОРТИРУЮТ игрока в зону
    // костра (сейв-точка всегда у костра), телепорт СИНХРОННО жмёт триггер костра
    // (OnSafeZoneBeginOverlap срабатывает прямо внутри SetActorLocationAndRotation), и его
    // автосейв перезаписывал слот тем, что есть В ПАМЯТИ НА ЭТОТ МОМЕНТ. Раньше рюкзак и
    // журнал восстанавливались ПОСЛЕ телепорта — в слот уезжало пустое состояние, а флаги
    // интро выживали (SaveGame переносит их из прежнего слота через CopyRetentionData).
    // Первый перезапуск играл по ещё живому слоту и сам его потрошил, второй читал
    // выпотрошенный — ровно симптом. Лечение двойное:
    //  1) замок bRestoringSaveState — костёр пропускает автосейв от телепорта восстановления
    //     (Campfire.cpp), SaveGame() отказывает второй линией;
    //  2) порядок: рюкзак/журнал/время восстанавливаются ДО телепорта — даже если какой-то
    //     новый путь запишет слот в этот момент, туда уедет уже полное состояние.
    bRestoringSaveState = true;
    ...
    // Статы + телепорт в сейв-точку (зона костра) — теперь, когда память уже полная.
    ApplySaveData(Save);
